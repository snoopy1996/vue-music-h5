<template>
  <div>
    <mu-appbar style="width: 100%;position: fixed;width: 100vw;top: 0;" color="primary">
      ...
      歌单
      <mu-button icon slot="right" @click="goSearch">
...
export default {
  ...
  async created() {
    ...
    this.playlistid = query.id;
    try {
      this.currentPlayList = await this.getlistDetail(this.playlistid);
    } catch (err) {}
    this.showProcess = false;
  },
  async activated() {
    const query = this.$route.query;
    if (query.id !== this.playlistid) {
      this.playlistid = query.id;
      this.showProcess = true;
      try {
        this.currentPlayList = await this.getlistDetail(this.playlistid);
  ...
  methods: {
    async getlistDetail(id) {
      const res = await this.$http.get(
        this.globarVar_.musicApi.playlist.detail,
        {
          params: { id, s: 5 }
        }
      );
      // console.log('getlistDetail-res.data', res.data.playlist);
      return res.data.playlist ? res.data.playlist : res.data.result;
    },
    back() {
      this.$router.back();
    },
